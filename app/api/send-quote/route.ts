...
export async function POST(request: Request) {
  try {
    ...
    const { name, email, phone, buildingType, serviceType, serviceLabel, surface, city, budget, budgetLabel, message, filesCount } = body;
...
${message ? `<tr><td style="padding:6px 12px;font-weight:600">Message</td><td>${message}</td></tr>` : ""}
    ...
    return NextResponse.json({ success: true, clientSent: clientOk, companySent: companyOk });
  } catch (err: any) {
